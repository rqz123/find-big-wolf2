...
SmartCam Watcher 是一个运行于 Windows 系统托盘的轻量级摄像头监控工具。它在指定工作时间内检测光线变化和局部移动，保存告警抓拍，并通过 Telegram Bot 将图片和时间戳发送到手机。
...
程序会在只有一个私人会话时自动取得并保存 `chat_id`，随后发送测试图片。如果机器人收到过多个私人用户的消息，程序会拒绝自动选择，避免把摄像头图片发错人；此时需要手动填写目标 `chat_id`。
...
6. 后台线程调用 Telegram `sendPhoto`，发送事件类型、当地时间和图片。
7. 网络超时、Telegram 限流或服务端错误会指数退避重试；配置或鉴权错误直接记录日志。
...
```text
find-big-wolf2/
├── core/                       摄像头、检测器和工作时间调度器
├── notify/telegram.py          Telegram Bot API 图片通知
├── ui/                         系统托盘和调试预览
├── utils/                      配置与日志
...
更详细的产品和工程约束见 [PRD v1.1](doc/SmartCam_Watcher_PRD_v1.1.md)。
